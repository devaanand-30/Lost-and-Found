...
const FoundForm = () => {
  ...
  const handlePreview = () => {
    const msg = `Someone reported a found item:\n\nTitle: ${formData.title}\nDescription: ${formData.description}\nLocation: ${formData.location}\nDate: ${formData.date}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/items/found', data, {
        headers: {
      ...
      await axios.post('/api/email/send', {
        to: formData.email,
        subject: `Found Item Reported: ${formData.title}`,
        text: previewMessage || `Found item reported: ${formData.description}`,
      });

      setSuccessMessage('Found item reported and email sent successfully!');
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto p-4">
      ...
      {successMessage && (
        <div className="text-green-600 text-center mb-2">{successMessage}</div>
      )}
      {errorMessage && (
